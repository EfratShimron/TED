function [dT,dT_zoomed] = TempCalc_v2(KspaceData_baseline,KspaceData_HIFU,S)
...
if S.FLAGS.dT_positivity==1
    % Here we apply a positivity constraint by replacing the values of all pixels in
    % which dT<=0 with 0. This constraint is applied only for a fair comparison with
    % the k-space Hybrid method [Gaur & Grissom 2015], and is not necessary in TED. 
    dT(dT<=0) = 0;
end
